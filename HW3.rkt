...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{= ANY ANY} s)
     (if_eqE (parse (second (s-exp->list s)))
             (parse (third (s-exp->list s))))]
...
(define (interp [a : Exp] [env : Env]) : Value
  (type-case Exp a
    ...
    [(if_eqE l r) (num= (interp l env) (interp r env))]
...
(module+ test
  ...
  (test (interp (parse `{if true 10 {+ 1 {lambda {x} x}}}) 
        ...
        (interp (parse `10) 
                mt-env))
  (test (interp (parse `{if #t 7 {+ 1 {lambda {x} x}}}) 
                mt-env) 
        (interp (parse `7) 
                mt-env))
  (test (interp (parse `{if #f 10 8}) 
                mt-env) 
        (interp (parse `8) 
                mt-env))
...
;; lookup ----------------------------------------
(define (lookup [n : Symbol] [env : Env])
  (type-case (Listof Binding) env
    [empty (error 'lookup "free variable")]
    [(cons b rst-env) (cond
                        [(symbol=? n (bind-name b)) (bind-val b)]
