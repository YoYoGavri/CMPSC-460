...
(define (override-store [storage : Storage] [sto : Store]) : Store
  (cond
    [(empty? sto) (cons storage mt-store)]
    [else (if (eq? (cell-location storage) (cell-location (first sto)))
          (cons storage (rest sto))
          (cons (first sto) (override-store storage (rest sto))))])) 
     ...
     (with [(v-b sto-b) (interp bx env sto)]
       (with [(v-v sto-v) (interp val env sto-b)]
         (type-case Value v-b
           ...
           [else (error 'interp "not a box")])))]
    [(beginE args)
     ;;(with [(v-l sto-l) (interp args env sto)]
     ;;  (interp args env sto-l))]))
     #;(cond
       [(> (length args) 0) (interp (first args) env sto)]
       [else
        (with [(v-l sto-l) (interp (first args) env sto)]
              (interp (rest args) env sto-l))])
     (beginE-helper args env sto)]))

(define (beginE-helper args env sto)
  (cond
    [(equal? (length args) 1) (interp (first args) env sto)]
    [(empty? args) (error 'box "Why you empty bruh")]
    [else (with [(v-l sto-l) (interp (first args) env sto)]
            (beginE-helper (rest args) env sto-l))]))
...
(module+ test
  ...
  (test/exn (fetch 2 (override-store (cell 3 (numV 9))
                                     mt-store))
            "unallocated location")

  (test (fetch 2 (override-store (cell 2 (numV 8))
                                 mt-store))
          (numV 8))
